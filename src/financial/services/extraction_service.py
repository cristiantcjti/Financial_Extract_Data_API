...
class ExtractionService:
    ...
    def extract_financial_data(
        ...
        cached_data = self.cache_service.get_cached_data("extraction", user_document)
        if cached_data:
            self.logger.info(f"Returning cached financial data for user_document: {user_document}")
            return FinancialDataResponseSchema(**cached_data)

        try:
            self.logger.info(f"Starting financial data extraction for user_document: {user_document}")
            ...
            balances_data = self._extract_balances(user_document, consent_data, accounts_data)
            self.logger.info(f"Balances extracted: {len(balances_data)}")

            transactions_data = self._extract_transactions(user_document, consent_data, accounts_data)
            self.logger.info(f"Transactions extracted: {len(transactions_data)}")
            ...
            self.logger.info(f"Financial data extraction completed successfully for user_document: {user_document}")
            
            self.cache_service.cache_data("extraction", user_document, formatted_response.model_dump())
            ...
            self.logger.error(f"Error in financial data extraction for user_document: {user_document}, Error: {str(e)}")
    ...
    def _get_or_create_consent(self, user_document: str, dynamic_client_id: str, token: str) -> ConsentData:
        try:
            ...
            raise ValueError(f"Failed to obtain consent: {str(e)}")

    def _fetch_paginated_data(self, route_class, route_data: dict[str, Any], data_key: str = "items") -> list[dict[str, Any]]:
        all_items = []
        page = 1
        has_next = True
        ...
        while has_next:
            ...
            try:
                ...
                if not result.success:
                    self.logger.warning(f"Paginated data extraction failed on page {page}, attempt {retry_count + 1}")
                    retry_count += 1
                    continue
                ...
                if data_key in response_json:
                    ...
                    self.logger.debug(f"Fetched page {page} with {len(page_items)} items, has_next: {has_next}")
                else:
                    all_items.append(response_json)
                    has_next = False
                    self.logger.debug(f"Fetched single item response on page {page}")
            ...
            page += 1
            if page > 100:
                self.logger.warning("Reached maximum page limit (100) to prevent infinite loop")
                break

        self.logger.info(f"Successfully fetched {len(all_items)} total items across {page - 1} pages")
        return all_items

    def _extract_accounts(self, user_document: str, consent_data: ConsentData) -> list[dict[str, Any]]:
        try:
            route_data = {
            ...
            accounts = self._fetch_paginated_data(AccountsRoute, route_data, "items")
            ...
            self.logger.error(f"Error extracting accounts for user_document: {user_document}, Error: {str(e)}")
            raise ValueError(f"Failed to extract accounts: {str(e)}")

    def _extract_balances(self, user_document: str, consent_data: ConsentData, accounts: list) -> list[dict[str, Any]]:
        try:
            balances = []
            for account in accounts:
                route = BalancesRoute(data=
                    {
                        "token": consent_data.token,
            ...
            self.logger.error(f"Error extracting balances for user_document: {user_document}, Error: {str(e)}")
            raise ValueError(f"Failed to extract balances: {str(e)}")

    def _extract_transactions(self, user_document: str, consent_data: ConsentData, accounts) -> list[dict[str, Any]]:
        try:
            all_transactions = []
            for account in accounts:
                route_data = {
                ...
                account_transactions = self._fetch_paginated_data(TransactionsRoute, route_data, "items")
                ...
                self.logger.info(f"Extracted {len(account_transactions)} transactions for account {account['id']}")

            return all_transactions

        except Exception as e:
            self.logger.error(f"Error extracting transactions for user_document: {user_document}, Error: {str(e)}")
    ...
    def _create_formatted_response(
        ...
        balance_lookup = {balance["account_id"]: balance["balance"] for balance in balances}
        ...
        for transaction in transactions:
            ...
            currency = balance_lookup.get(account_id, {}).get("currency", "BRL")
        ...
        for account in accounts:
            account_id = account["id"]
            account_balance = balance_lookup.get(account_id, {"amount": 0.0, "currency": "BRL"})
            ...
            account_data = {
                ...
                "account_status": account.get("account_status", "UNKNOWN").upper(),
        ...
        total_transactions = sum(len(account_transactions) for account_transactions in transactions_by_account.values())
    ...
    def _convert_account_data(self, account_data: dict[str, Any]) -> AccountSchema:
    ...
    def _convert_transaction_data(self, transaction_data: dict[str, Any]) -> TransactionSchema:
