...
class TestBaseRoute:
    ...
    def test_execute_request_with_headers(self, route, mock_response):
        # Arrange
        route.get_authorization_header = Mock(
    ...
    def test_integrate_method_mapping(
        self, route, mock_response, method, expected_function
    ):
        # Arrange
        route.method = method
        ...
        with patch.object(
            route, expected_function, return_value=mock_response
        ) as mock_method:
            # Act
            result = route.integrate()
            # Assert
